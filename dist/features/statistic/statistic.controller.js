...
let statsCache = null;
function decodeBasicAuthorizationHeader(authHeader) {
...
export async function getStatistic(req, res) {
    ...
    const usersDetailed = users.map((user) => {
        ...
        const notificationsTotal = notificationsByCurrentUser.PENDING +
...
export async function manageDrugarski(req, res) {
    if (!isAuthorized(req, res))
        return;
