...
export function markIngestHeartbeat() {
    lastHeartbeatAt = Date.now();
}
function killStrayChromium() {
...
export function startIngestWatchdog() {
    ...
    watchdogTimer = setInterval(() => {
        ...
        if (staleMs <= STALE_MS)
            return;
        console.error(`[watchdog] INGEST ZAGLAVLJEN — nijedan ciklus nije zavrsen ${Math.round(staleMs / 60_000)} min (prag: ${MISSED_CYCLES_THRESHOLD} propustenih ciklusa = ${Math.round(STALE_MS / 60_000)} min). Forsiram restart procesa (PM2 ce ga automatski podici nazad).`);
        killKpBrowser();
        killPaBrowser();
        killStrayChromium();
    ...
    watchdogTimer.unref();
    console.log(`[watchdog] Ingest watchdog pokrenut — restart posle ${MISSED_CYCLES_THRESHOLD} propustenih ciklusa (${Math.round(STALE_MS / 60_000)} min bez zavrsenog ciklusa).`);
}
