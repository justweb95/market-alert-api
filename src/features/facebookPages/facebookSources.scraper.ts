...
type ApifyMarketplaceItem = Record<string, unknown>;
...
function getIdFromApifyItem(item: ApifyMarketplaceItem): string | null {
  const candidates = [
    item.id,
    item.listingId,
    item.listing_id,
...
async function scrapeFacebookMarketplaceViaApify(input: {
  ...
  const actorId =
    process.env.FB_MARKETPLACE_APIFY_ACTOR_ID?.trim() ||
  ...
  const proxyCountry =
    process.env.FB_MARKETPLACE_PROXY_COUNTRY?.trim().toUpperCase() || "RS";
  if (proxyCountry) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `[fb-marketplace] Apify ${response.status}: ${errorText || response.statusText}`,
    );
  }

  const payload = (await response.json()) as ApifyMarketplaceItem[];
