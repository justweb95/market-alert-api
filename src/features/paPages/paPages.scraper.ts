...
function splitTitlePriceRest(alt: string): { title: string; priceEur: string; rest: string } | null {
  const m = alt.match(/^(.*?),\s*([\d.,]+)\s*€,?\s*(.*)$/s);
  if (!m) return null;
  return { title: (m[1] ?? '').trim(), priceEur: `${(m[2] ?? '').trim()} €`, rest: (m[3] ?? '').trim() };
}

function lastCsvSegment(text: string): string {
...
export async function scrapePaLatestCars(opts?: { take?: number; timeoutMs?: number }) {
  ...
  $(ARTICLE_SELECTOR).each((_, el) => {
    try {
      ...
      const detail = parsed.rest.match(
        /^(\d{4})\.\s*([^,]*),\s*([^|]+)\|\s*(\d+)\s*cm3,\s*([\d.,]+)\s*km,\s*\d+kW\s*\(\d+\s*KS\),\s*[^,]+,\s*\d+\/\d+\s*vrata,\s*\d+\s*sedišta,\s*(.+)$/
      );
...
export async function scrapePaLatestMotos(opts?: { take?: number; timeoutMs?: number }) {
  ...
  $(ARTICLE_SELECTOR).each((_, el) => {
    try {
      ...
      const detail = parsed.rest.match(
        /^(\d{4})\.\s*,\s*(\d+)\s*cm3,\s*([\d.,]+)\s*km,\s*(\d+kW\s*\(\d+\s*KS\)),\s*([^,]+),\s*(.+)$/
      );
  ...
  $(ARTICLE_SELECTOR).each((_, el) => {
    try {
      ...
      const listingType: 'MOTO_PART' | 'MOTO_EQUIPMENT' =
        /(oprema|kaciga|jakna|rukavice|cizme|protektor)/.test(combinedText)
          ? 'MOTO_EQUIPMENT'
          : 'MOTO_PART';
